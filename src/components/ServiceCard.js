...
import { useRouter } from "next/navigation";
import { FaIdCard } from "react-icons/fa"; // Import icon for driver’s licenses
import { formatPrice } from "../utils/formatPrice";
...
export default function ServiceCard({ service, selectedId, onSelect }) {
  ...
  const handleClick = () => {
    ...
    setTimeout(() => {
      router.push(
        `/service_request_form?category=driver_license&id=${service.id}&price=${service.price}`
      );
    }, 100);
  };
  ...
  return (
    <button
      onClick={handleClick}
      aria-label={`Select Driver License - ${service.name}`}
      className={`cursor-pointer border rounded-xl p-6 shadow hover:shadow-lg transition items-center text-center
        ${isSelected ? "bg-black text-white" : "bg-gray-100 border-green-400"}`}
    >
      <div className="flex flex-col items-center">
