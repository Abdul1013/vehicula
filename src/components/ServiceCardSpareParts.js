...
export default function ServiceCard({ service, selectedId, onSelect }) {
  ...
  const handleClick = () => {
    ...
    setTimeout(() => {
      ...
      const url = `${formPath}?category=${encodeURIComponent(
        service.category || "driver_license"
      )}&id=${service.id}&price=${service.price}&name=${encodeURIComponent(
        service.name
      )}`;
  ...
  return (
    <button
      onClick={handleClick}
      aria-label={`Select ${service.category || "Driver License"} - ${
        service.name
      }`}
